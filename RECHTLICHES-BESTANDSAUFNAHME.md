...
| 6a | **Rate-Limiting** auf `get_party_by_invite_code` — **ERLEDIGT am 27.08.2026.** Nicht über Cloudflare: dort sind wir nicht Kunde, die Instanz gehört Supabase. Und nicht über die eigene Domain: der anon-Key liegt im Browser-Bundle, jeder kann die REST-API direkt ansprechen. Die Bremse sitzt deshalb in der Funktion selbst — 30 Fehlversuche je IP und Minute, gezählt in `private.invite_lookup_misses`, Abweisung mit HTTP 429. Gegen die Live-API geprüft: 30× 200, danach 429 | Migrationen `20260827104334`, `20260827104435` |
...
**Am 27.08.2026 geklärt:** Der Build-Fehler `Export encountered an error on /_global-error/page`, `TypeError: Cannot read properties of null (reading 'useContext')` lag weder an den Rechtstexten noch an der App. Ursache war `NODE_ENV=development` aus `.claude/settings.json` — der Wert galt für jede Sitzung, und `next build` mischt damit Entwicklungs- und Produktionsbuild von React, bis der Server-Renderer ohne Dispatcher dasteht. Nachgewiesen an einem Minimalprojekt (zwei Dateien, frische Installation, dieselben Versionen): scheitert identisch, und mit `env -u NODE_ENV` baut es durch. Der Eintrag ist entfernt, `npm run build` baut alle 26 Routen. Für Vercel war das nie ein Hindernis — dort steht `NODE_ENV` beim Build auf `production`.
